...
public class OracleConnector extends GenericDataBaseConnector
{
	public static final String SERVER_PROP_KEY = "server";
	public static final String SID_PROP_KEY = "sid";
	...
	private OracleSchemaInspector inspector;

	public OracleConnector(Properties conf)
	{
		super(conf);

		String server = conf.getProperty(SERVER_PROP_KEY);
		String sid = conf.getProperty(SID_PROP_KEY);
		...
		try
		{
			this.inspector = new OracleSchemaInspector(server, sid, user, pass, port);
	...
	public List<Table> allTablesWithAssociationsByOwner(String owner) throws ConnectorException
	{
		List<Table> result = null;
		try {
			result = this.inspector.allTablesWithAssociationsByOwner(owner);
		} catch (SQLException e) {
			throw new ConnectorException(e);
		}
	...
	public List<Table> allTablesWithAssociations() throws ConnectorException
	{
		List<Table> result = null;
		try {
			result = this.inspector.allTablesWithAssociations();
		} catch (SQLException e) {
			throw new ConnectorException(e);
		}
	...
	public List<Table> allTablesByOwner(String owner) throws ConnectorException
	{
		List<Table> result = null;
		try {
			result = this.inspector.allTablesByOwner(owner);
		} catch (SQLException e) {
			throw new ConnectorException(e);
		}
