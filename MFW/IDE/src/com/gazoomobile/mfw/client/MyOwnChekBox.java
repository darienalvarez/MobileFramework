 ...
 * @author Yunisel
 */
public class MyOwnChekBox extends JCheckBox{
    private JPrincipal owner;
    private List<ControlProperty> properties;

    public MyOwnChekBox(String text, JPrincipal owner) {
        super();
        setText(text);
        this.owner = owner;
        ...
        try {
          control = new ControlWebServiceImpl(new URI("http://localhost:8080/mfw/services/ControlService?wsdl").toURL()).getControlWebServiceImplPort();
            ...
            Logger.getLogger(MyOwnChekBox.class.getName()).log(Level.SEVERE, null, ex);
